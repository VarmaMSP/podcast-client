...
export async function selectFeed (podcastId: number) {
  let db = await openDB()
  return getValue(db.transaction(['feed']).objectStore('feed'), podcastId)
}

export async function deleteFeed (podcastId: number) {
  let db = await openDB()
  return deleteValue(db.transaction(['feed'], 'readwrite').objectStore('feed'), podcastId)
}

export async function insertFeed (feed: FeedScheme) {
  let db = await openDB()
  return updateValue(db.transaction(['feed'], 'readwrite').objectStore('feed'), feed)
}
